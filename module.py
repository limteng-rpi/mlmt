...
def one_hot(index, classes):
    ...
    mask = torch.cuda.FloatTensor(*size).fill_(0)
    index = index.view(*view)
    ones = 1.

    if isinstance(index, Variable):
        ones = Variable(torch.cuda.FloatTensor(index.size()).fill_(1))
...
class LSTM(nn.LSTM):
    ...
    # def cuda(self, device=None):
    #     return super(LSTM, self).cuda(device)
    #
    # def cpu(self, device=None):
    #     return super(LSTM, self).cpu(device)
