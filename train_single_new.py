...
from model import Linear, Linears, LSTM, CRF, CharCNN, Highway, LstmCrf, load_embedding
...
argparser.add_argument('--charcnn_layer', default=1, type=int)
argparser.add_argument('--charcnn_func', default='relu')
...
char_embed = CharCNN(len(char_vocab),
                     args.char_embed_dim,
                     filters=[[2, 25], [3, 25], [4, 25]])
char_hw = Highway(char_embed.output_size,
                  layer_num=args.charcnn_layer,
                  activation=args.charcnn_func)
...
state = {
    'model': {
        ...
        'char_cnn': char_embed.state_dict(),
        'char_highway': char_hw.state_dict(),
        'lstm': lstm.state_dict(),
        'crf': crf.state_dict(),
        'output_linear': linear.state_dict(),
