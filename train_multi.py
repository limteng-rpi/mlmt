...
argparser.add_argument('--test_tgt',
                       help='Path to the test set file of the target task')
...
argparser.add_argument('--test_cl',
                       help='Path to the test set file of auxiliary task 1 (same task, different language)')

argparser.add_argument('--train_ct',
                       help='Path to the training set file of auxiliary task 2 (different task, same language)')
argparser.add_argument('--dev_ct',
                       help='Path to the dev set file of auxiliary task 2 (different task, same language)')
argparser.add_argument('--test_ct',
                       help='Path to the test set file of auxiliary task 2 (different task, same language)')

argparser.add_argument('--train_clct',
                       help='Path to the training set file of auxiliary task 3 (different task, different language)')
argparser.add_argument('--dev_clct',
                       help='Path to the dev set file of auxiliary task 3 (different task, different language)')
argparser.add_argument('--test_clct',
                       help='Path to the test set file of auxiliary task 3 (different task, different language)')
...
# assert embed_file1 and embed_file2, 'Embedding file is required'
...
pos_tagging_parser = ConllParser(Config({
'separator': '\t',
    'token_col': 1,
    'label_col': 3,
    'skip_comment': True,
...
test_set_tgt = SequenceDataset(Config({
    'path': args.test_tgt,
    'parser': name_tagging_parser,
    'batch_size': batch_size
}))
...
test_set_cl = SequenceDataset(Config({
    'path': args.test_cl,
    'parser': name_tagging_parser,
    'batch_size': batch_size
}))
train_set_ct = SequenceDataset(Config({
    'path': args.train_ct,
    'parser': pos_tagging_parser,
    'batch_size': batch_size
}))
dev_set_ct = SequenceDataset(Config({
    'path': args.dev_ct,
    'parser': pos_tagging_parser,
    'batch_size': batch_size
}))
test_set_ct = SequenceDataset(Config({
    'path': args.test_ct,
    'parser': pos_tagging_parser,
    'batch_size': batch_size
}))
train_set_clct = SequenceDataset(Config({
    'path': args.train_clct,
    'parser': pos_tagging_parser,
    'batch_size': batch_size
}))
dev_set_clct = SequenceDataset(Config({
    'path': args.dev_clct,
    'parser': pos_tagging_parser,
    'batch_size': batch_size
}))
test_set_clct = SequenceDataset(Config({
    'path': args.test_clct,
    'parser': pos_tagging_parser,
    'batch_size': batch_size
}))
...
shared_output_linear_2 = Linear(Config({
    'in_features': lstm.output_size,
    'out_features': len(label_vocab_2)
}))
spec_output_linear_2_1 = Linear(Config({
    'in_features': lstm.output_size,
    'out_features': len(label_vocab_2)
}))
spec_output_linear_2_2 = Linear(Config({
    'in_features': lstm.output_size,
    'out_features': len(label_vocab_2)
}))
...
lstm_crf_ct = LstmCrf(
    ...
    crf=crf_2,
    lstm=lstm,
    univ_fc_layer=shared_output_linear_2,
    spec_fc_layer=spec_output_linear_2_1,
    embed_dropout_prob=args.embed_dropout,
    lstm_dropout_prob=args.lstm_dropout,
    linear_dropout_prob=args.linear_dropout,
    char_highway=char_highway
...
lstm_crf_clct = LstmCrf(
    ...
    lstm=lstm,
    univ_fc_layer=shared_output_linear_2,
    spec_fc_layer=spec_output_linear_2_2,
    embed_dropout_prob=args.embed_dropout,
    lstm_dropout_prob=args.lstm_dropout,
    linear_dropout_prob=args.linear_dropout,
    char_highway=char_highway
